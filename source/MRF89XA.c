...
void remote_setup(void) {
   ...
   * Enable packet mode
   * Set gain to 0 dB */
  remote_config_raw(RF_DMODREG, RF_MODSEL_FSK | RF_DMODE_PCKT | RF_IFGAIN_0DB);
  /* Frequency deviation 
   * Fdev = Fxtal / (32 * (1 + FDVAL))
   * => 40 kHz
   * Max allowed 40 kHz */
  remote_config_raw(RF_FDEVREG, 0x09);
  /* Bit rate
   * BR = Fxtal / (64 * (1 + BRVAL))
   * => 25 kbps 
   * Max. allowed 40 kbps in FSK mode */
  remote_config_raw(RF_BRSREG, 0x07);
   ...
   * P = 79
   * S = 28 */
  remote_config_raw(RF_R1CREG, (uint8_t)(99));
  remote_config_raw(RF_P1CREG, (uint8_t)(79));
  remote_config_raw(RF_S1CREG, (uint8_t)(28));
  /* Interrupt configuration
   * IRQ0 - RX: Payload Ready, i.e. packet has arrived
   * IRQ1 - RX: Fifo overload
   * IRQ1 - TX: Tx done */
  remote_config_raw(RF_FTXRXIREG, RF_IRQ0RXS_PLREADY | RF_IRQ1RXS_FIFOFULL | RF_IRQ1TX_TXDONE);
  /* Fifo configuration:
   * Send when first byte is in fifo, IRQ0 when fifo is not empty */
  remote_config_raw(RF_FTPRIREG, RF_IRQ0TXST_EMPTY | RF_FRPRI_RESERVED);
  /* TODO: Filter configuration */
  /* Single sideband bandwidth of low pass filter
   * Set to 378 kHz
   * Butterworth filter: 100 kHz */
  remote_config_raw(RF_FILCREG, RF_PASFILV_378KHZ | RF_BUTFILV_100KHZ);
  /* Polyphase filter
   * 100 kHz */
  remote_config_raw(RF_PFCREG, RF_POLCFV_100KHZ | RF_PFC_RESERVED);
  /* Sync control
   * Enable sync word recognition
   * Set sync word size to 16 bits
   * Set sync word error tolerance to 0 bits */
  remote_config_raw(RF_SYNCREG, RF_SYNCREN_ON | RF_SYNCWSZ_16 | RF_SYNCTEN_0);
  /* Sync word
   * 0x5555 */
  remote_config_raw(RF_SYNCV31REG, RF_SYNCWRD_3);
  remote_config_raw(RF_SYNCV23REG, RF_SYNCWRD_2);
  remote_config_raw(RF_SYNCV15REG, RF_SYNCWRD_1);
  remote_config_raw(RF_SYNCV07REG, RF_SYNCWRD_0);
  /* TX interpolation filter settings
   * Cut off 125 kHz
   * Fc = 200 * TXIPOLFV/8
   * => 5
   * TX output power: 10 dBm */
  remote_config_raw(RF_TXCONREG, RF_TXIPOLFV_125KHZ | RF_TXOPVAL_10DBM);
  /* Disable clock output */
  remote_config_raw(RF_CLKOREG, RF_CLKOCNTRL_OFF);
  /* Disable Manchester encoding
   * Max. payload length in RX mode: 20 bytes */
  remote_config_raw(RF_PLOADREG, RF_MCHSTREN_OFF | RF_MAX_PLLEN);
